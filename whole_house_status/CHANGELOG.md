# 更新日志

## 2.0.0 - 2026-09-18

- 界面重构：新增浅色主题并自动跟随系统深浅色模式切换（prefers-color-scheme）。
- 统计行改为单卡片分格布局（上标签下数字），560px 以下压缩为单行，与「日志分析与修复」面板风格统一。
- 页面容器宽度调整为 1378px，标题区去掉分割线，间距整体微调。
- 动效与状态色优化（绿色 #19be5d、橙色 #f97316、红色 #e8453c）。

## 1.1.0 - 2026-09-18

- 仪表盘标题改为 22px、字重 600、白色、0.5px 字间距，副标题改为 12px 灰色（#9a9a9a），桌面端与移动端一致。

## 1.0.4 - 2026-09-18

- 移动端排序与显示两个圆形按钮跟随房间筛选网格自然排布：未分组后有空位时同行紧随其后，仅当一行三格占满时才换行显示。

## 1.0.3 - 2026-09-18

- 状态事件突发（如 Zigbee 网络抖动）时合并为一次广播，150ms 防抖避免逐条触发全量快照，降低树莓派 CPU 尖峰。
- 快照只克隆监控域内实体与高功率告警引用的功率传感器，不再每秒克隆 Home Assistant 全部状态。
- 移除布尔值 watchdog 字段：新版 Supervisor（HA OS 18.x 应用架构）要求其为 URL 字符串，布尔值会导致商店无法解析插件配置。

## 1.0.2 - 2026-09-17

- 仪表盘标题改为左对齐，并在标题下方新增「Home Assistant 全屋设备在线状态」小字副标题，桌面端与移动端一致。

## 1.0.1 - 2026-09-17

- 改用 ghcr.io 预构建镜像，更新不再需要设备本地构建，避免国内网络拉取基础镜像缓慢的问题。
- 新增 GitHub Actions 工作流，推送代码后自动构建并发布对应版本的镜像。

## 1.0.0 - 2026-09-17

- 跳过内容未变化的仪表盘广播，空闲连接不再每秒收到相同快照，且每次广播只序列化一次供所有客户端使用。
- 全量状态同步时，即使 Home Assistant 返回无效条目，也保留其余有效状态。
- 软件包版本与插件发布版本保持一致。
- 移除测试工具与开发检查，精简仓库。

## 0.1.27 - 2026-07-28

- 插件与自定义仓库更名为「全屋设备状态」。

## 0.1.26 - 2026-07-28

- 激活中的漏水传感器显示为红色「漏水」告警，替代通用的「故障」标签。

## 0.1.25 - 2026-07-26

- 当设备、实体或区域分配变更时自动刷新 Home Assistant 注册表，房间分组无需重启插件即可更新。

## 0.1.24 - 2026-07-20

- 鼠标悬停时保持所选房间的高亮状态。
- 房间排序与显示控件采用仅背景色的悬停反馈。

## 0.1.23 - 2026-07-20

- 单次排序会话中允许多次调整房间顺序，关闭排序控件时保存最终顺序。
- 房间筛选悬停反馈改为仅背景色变化，不再高亮边框。

## 0.1.22 - 2026-07-19

- 显示菜单按文档可用视口定位，避免被桌面滚动条遮挡。
- 显示菜单边框和标签保持在可用视口内，包括窄屏或矮屏。

## 0.1.21 - 2026-07-19

- 显示菜单自动定位在视口内，包括筛选控件位于边缘或视口较矮的情况。

## 0.1.20 - 2026-07-19

- 移动端房间排序与显示控件保持相邻，显示菜单不再超出视口。

## 0.1.19 - 2026-07-19

- 修复运行时房间顺序保存失败的问题：更新插件配置前先解包 Supervisor options 响应。

## 0.1.18 - 2026-07-19

- 仪表盘房间筛选支持拖拽排序，结果通过 Supervisor API 持久化为 `rooms.order`。
- 运行时房间排序保持「全部」在首位、「未分组」在末位。

## 0.1.17

- 为插件配置字段添加简体中文标签与描述。
- 移除开发计划与测试工具，精简源码包。

## 0.1.16

- 将已忽略实体卡片与活动警告、错误分开显示。

## 0.1.15

- 移动端房间筛选网格与统计卡片对齐，显示图标保持在最后一个房间旁。

## 0.1.14

- 显示已忽略实体的选项改用紧凑的自定义复选框。

## 0.1.13

- 显示控件改为圆形图标按钮。

## 0.1.12

- 未分组房间保持在末位，对齐移动端房间控件，改进显示与卡片可读性。

## 0.1.11

- 已忽略实体显示选项按内容自适应宽度。

## 0.1.10

- 减小仪表盘顶部间距，压缩已忽略实体显示菜单。

## 0.1.9

- 移动端房间选择控件在末行未占满时保持固定宽度。

## 0.1.8

- 在不改变文字大小的前提下压缩房间选择与显示控件边框。

## 0.1.7

- 新增仅作用于仪表盘的实体忽略操作并持久化，不改变实体在 Home Assistant 中的可见性。

## 0.1.6

- 发布更新后的 Home Assistant 插件包。

## 0.1.5

- 添加 Supervisor 更新所需的插件更新日志。

## 0.1.4

- 减小仪表盘标题字号。

## 0.1.3

- 标准桌面宽度下保持每行五张实体卡片。

## 0.1.2

- 采用紧凑的水平统计卡片与五列桌面实体网格。

## 0.1.1

- 在 Home Assistant 中隐藏的实体，在仪表盘中视为已忽略。

## 0.1.0

- Home Assistant 插件首个发布版本。
